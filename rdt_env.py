...
class Rdt(gym.Env):
    ...
    # def set_allocation_class(self, action_be_ways):
...
#
#         # pqos
#         self.pqos_interface = config[PQOS_INTERFACE]
#         cores_pid_hp_range = parse_num_list(config[CORES_LC])
#
#         self.scheduler = None
#         # scheduler
#         self.cores_per_be = int(config[CORES_PER_BE])
#         self.cores_pids_be = config[CORES_BE]
...
#         self.be_name = ast.literal_eval(config[BES_LIST]) if config[BES_LIST] else None
...
#         seconds = int(round((self.interval_bes % 1) * 60, 0))
